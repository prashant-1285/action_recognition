...
def parse_args():
    ...
    parser.add_argument(
        '--config',
        default='configs/posec3d/slowonly_r50_346_k400/joint.py',
        help='skeleton action recognition config file path')
    parser.add_argument(
        '--checkpoint',
        default='/home/prashant/Documents/legion/personal_projects/action/pyskl/ckpts/slowonly_r50_8xb32-u48-240e_k400-keypoint_20230731-7f498b55.pth',
    ...
    parser.add_argument(
        '--label-map',
        default='tools/data/label_map/k400.txt',
        help='label map file')
    parser.add_argument(
...
def main(video_in_path,video_out_path,action_config,action_checkpoint,action_labelmap):
    args = parse_args()

    frame_paths, original_frames = frame_extraction(video_in_path,
                                                    args.short_side)
    num_frame = len(frame_paths)
    h, w, _ = original_frames[0].shape

    config = mmcv.Config.fromfile(action_config)
    ...
    model = init_recognizer(config, action_checkpoint, args.device)

    # Load label_map
    label_map = [x.strip() for x in open(action_labelmap).readlines()]
